...
vector<vector<int>> dp(10000, vector<int>(4, -1));

int getMin(vector<int> &steps, int currLane, int currStep)
{
    ...
    for (int i = 1; i <= 3; i++)
    {
        if (steps[currStep] != i && steps[currStep + 1] != i)
        {
            if (i == currLane)
                ans = min(ans, getMin(steps, i, currStep + 1));
            else
                ans = min(ans, 1 + getMin(steps, i, currStep + 1));
        }
    }
    return dp[currStep][currLane] = ans;
}
...
int main()
{
    ...
    int currLane = 2, currStep = 0;
    int ans = getMin(steps, currLane, currStep);
