...
int pickOrLeave(vector<int> &arr, int idx, int d)
{
    int res = 1;
    int n = arr.size();
    for (int i = idx + 1; i < n; i++)
    {
        ...
        if (arr[i] - arr[idx] == d)
        {
            // cout << "Found: " << arr[i] << " " << arr[idx] << endl;
            int next = 1 + pickOrLeave(arr, i, d);
            res = max(res, next);
        }
    }
    // cout << endl;
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        res = max(res, pickOrLeave(arr, i, d));
    }
    // int ans = pickOrLeave(arr, 0, d);
    cout << res << endl;
    return 0;
}